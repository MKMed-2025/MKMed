...
from dataloader import KgDataset, DescDataset, ChemDataset, ThreeDDataset, ImgDataset
from mole_align import KG_align, Img_align, Desc_align, Chem_align, ThreeD_align
from utils import llprint, multi_label_metric, ddi_rate_score, get_n_params, parameter_report, Regularization

def KgTrain(mole_encoder, clip_net, loss_log_path, args):
    if args.debug:
    ...
    model = KG_align(mole_encoder, clip_net, args.dim, args.device)
    ...
    for epoch in range(EPOCH):
        ...
        if epoch_loss < best_loss:
            ...
            best_model = model.state_dict()
            logger.info(f"Epoch {epoch + 1}: 最佳模型已保存，损失值为: {best_loss}")
    ...
    return model.mole_encoder

def ImgTrain(mole_encoder, clip_net, preprocess, loss_log_path, args):
    if args.debug:
    ...
    model = Img_align(mole_encoder, clip_net, args.dim, args.device)
...
def DescTrain(mole_encoder, clip_net, loss_log_path, args):
    if args.debug:
    ...
    model = Desc_align(mole_encoder, clip_net, args.dim, args.device)
...
def ChemFeatureTrain(mole_encoder, clip_net, loss_log_path, args):
    if args.debug:
    ...
    model = Chem_align(mole_encoder, clip_net, args.dim, args.device)
...
def ThreeDTrain(mole_encoder, clip_net, loss_log_path, args):
    if args.debug:
    ...
    model = ThreeD_align(mole_encoder, clip_net,args.dim, args.device)
