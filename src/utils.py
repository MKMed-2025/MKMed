...
from modules.gnn.mol_graph import sdf_to_graphs, sdf_to_graphs_list
...
def ddi_rate_score(record, args):
    # ddi rate
    path = f'data/{args.dataset}/output/ddi_A_final.pkl'
...
def parameter_report(best, regular,args):
    """输出一个参数的文件"""
    with open(f"{args.train_saved_path}/{args.dataset}/parameter_report_{args.time}.txt", 'w+') as f:
        f.write(
...
def graph_report(history, args):
    ...
    plt.legend()
    plt.savefig(f'{args.train_saved_path}/{args.dataset}/jaccard_6-1_lr=0.0005_layer=4.png')
...
def weighted_cross_entropy(logits_img2desc, logits_img2motif, logits_desc2motif, ground_truth):
    loss_func = nn.CrossEntropyLoss()
    # 计算图片、描述和 motif 的交叉熵损失
    loss_img_desc = (loss_func(logits_img2desc, ground_truth) + loss_func(logits_img2desc.t(), ground_truth)) / 2
    loss_img_motif = (loss_func(logits_img2motif, ground_truth) + loss_func(logits_img2motif.t(), ground_truth)) / 2
    loss_desc_motif = (loss_func(logits_desc2motif, ground_truth) + loss_func(logits_desc2motif.t(), ground_truth)) / 2


    weight_img_desc = 0.5
    weight_img_motif = 0.3
    weight_desc_motif = 0.2


    total_loss = weight_img_desc * loss_img_desc + weight_img_motif * loss_img_motif + weight_desc_motif * loss_desc_motif

    return total_loss


def gen_sub_projection(smiles_list,sub_list):

    sub_list = [ast.literal_eval(item) for item in sub_list]
    fracSet = []
    for i in sub_list:
        fracSet += i
        fracSet = list(set(fracSet))

    projection_matrix = np.zeros((len(smiles_list), len(fracSet)))
    for i, fracList in enumerate(sub_list):
        for frac in fracList:
            projection_matrix[i, fracSet.index(frac)] = 1

    return fracSet,projection_matrix
...
def drug2sdf_file(sdf_files):

    drug2sdf_map = {Path(f).stem: f for f in sdf_files}


    if all(Path(f).stem.isdigit() for f in sdf_files):
        drug2sdf_map = {int(Path(f).stem): f for f in sdf_files}
