...
with open(ddi_mask_path, 'rb') as Fin:
    ddi_mask_H = torch.from_numpy(dill.load(Fin)).to(args.device)
...
mole_encoder = GNNGraph(num_layer=3,emb_dim=args.dim,drop_ratio=args.dp,virtual_node=False,device=args.device).to(args.device)

# clip_net, preprocess = clip.load("ViT-B/32", device=args.device, jit=False)
# mole_encoder = KgTrain(mole_encoder, clip_net, loss_log_path, args)
# mole_encoder = ChemFeatureTrain(mole_encoder, clip_net, loss_log_path, args)
# mole_encoder = ImgTrain(mole_encoder, clip_net, preprocess, loss_log_path, args)
# mole_encoder = DescTrain(mole_encoder, clip_net, loss_log_path, args)
# mole_encoder = ThreeDTrain(mole_encoder, clip_net, loss_log_path, args).to(args.device)

model = MKMed(
    graph=graph,
    mole_encoder=mole_encoder,
